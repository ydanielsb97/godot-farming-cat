...
@onready var feed_component: Feed = $FeedComponent
...
func _unhandled_input(event: InputEvent) -> void:
	if in_range and event.is_action_pressed("show_dialogue"):
		...
		# Create dialogue for the chest
		var balloon: BaseGameDialogueBalloon = balloon_scene.instantiate()
		get_tree().root.add_child(balloon)
		balloon.start(load("res://dialogues/conversations/chest.dialogue"), dialogue_start_command)
...
func add_reward_scene() -> void:
	for scene in output_reward_scenes:
		...
		var reward_position: Vector2 = get_random_position_in_circle(reward_marker.global_position, reward_output_radious)
